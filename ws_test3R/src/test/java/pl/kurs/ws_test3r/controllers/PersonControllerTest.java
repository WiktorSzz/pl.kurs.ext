    ...
    void shouldFailToEditPersonWithIncorrectVersion() throws Exception {
        ...
        mockMvc.perform(put("/api/v1/persons/{id}", 8L)
                ...
                .andExpect(jsonPath("$.message")
                        .value("Optimistic locking failed: Version mismatch: expected version 0, but got 4"));
    }


    @Test
    @WithMockUser(username = "admin", roles = {"ADMIN"})
    void shouldFailToCreatePersonWithDuplicateEmail() throws Exception {
        CreateEntityCommand duplicateEmailCommand = new CreateEntityCommand(
                "Student",
                Map.of(
                        "firstName", "Duplicate",
                        "lastName", "Email",
                        ...
                        "height", "160.0",
                        "weight", "50.0",
                        "email", "pawel.chmielewski1@testmail.com",
                        "graduatedUniversity", "AGH",
                        "yearOfStudy", "3",
                        "fieldOfStudy", "Math",
                        "scholarship", "1500.0"
                )
        ...
        String requestJson = objectMapper.writeValueAsString(duplicateEmailCommand);

        mockMvc.perform(post("/api/v1/persons")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(requestJson))
                .andExpect(status().isConflict())
                .andExpect(jsonPath("$.errorsList[0].message").value("Data integrity violation occurred."));
    }


    @Test
    @WithMockUser(username = "admin", roles = {"ADMIN"})
    void shouldReturnPersonsFilteredByHeightRange() throws Exception {
        mockMvc.perform(get("/api/v1/persons")
                        .param("type", "employee")
                        .param("heightFrom", String.valueOf(160.0))
                        .param("heightTo", String.valueOf(162.0)))
                .andExpect(status().isOk())

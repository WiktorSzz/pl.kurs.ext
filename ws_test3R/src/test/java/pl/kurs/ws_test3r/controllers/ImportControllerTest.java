...
public class ImportControllerTest {
    ...
    public void shouldRollbackTransactionWhenInvalidDataIsEncountered() throws Exception {
        ...
        Mockito.doThrow(new ImportProcessingException("Unsupported entity type"))
                .when(importService).processEntityFile(Mockito.any(), Mockito.anyString());
    ...
    @Test
    @WithMockUser(username = "admin", roles = {"ADMIN"})
    public void shouldImportEmployeeAndVerifyCallToService() throws Exception {
        MockMultipartFile file = new MockMultipartFile(
                "file",
                "employees.csv",
                "text/plain",
        ...
        Mockito.doNothing().when(importService).processEntityFile(Mockito.any(), Mockito.anyString());
        ...
        Mockito.verify(importService, Mockito.times(1)).processEntityFile(Mockito.any(), Mockito.eq("employees.csv"));
    }
